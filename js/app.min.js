...
boothApp.run([
  'questionsAPI', '$rootScope', '$state', '$ionicPlatform', function(questionsAPI, $rootScope, $state, $ionicPlatform) {
    ...
    return $ionicPlatform.ready(function() {
      if (!($rootScope.online = navigator.onLine)) {
        return alert("offline mode");
      }
    });
  }
]);
  ...
  "$http", "CacheFactory", "$q", function($http, CacheFactory, $q) {
    ...
    return {
      ...
      question: function(id) {
        ...
        deferred = $q.defer();
        questionPromise = $http.get(apiURL + "question&lang=" + this.filter().language + "&id=" + id, {
          cache: caches.memory
        });
        $q.all([this.questions(), questionPromise]).then(function(arg) {
          var k, len, metadata, question, questionResponse, questionsResponse, ref;
      ...
      filterQuestions: function(filter, useCache) {
        ...
        if (useCache && caches.memory.get("filteredQuestions")) {
          ...
          this.questions().then(function(response) {
            var card, filteredQuestions, hasIllegalCard, i, isLegalCard, j, k, l, len, len1, len2, m, question, questions, ref, ref1, ref2, set, t;
            questions = response.data;
            filteredQuestions = [];
            for (k = 0, len = questions.length; k < len; k++) {
              ...
              if (filter.sets.length) {
                ...
                for (l = 0, len1 = ref2.length; l < len1; l++) {
                  card = ref2[l];
                  isLegalCard = false;
                  for (m = 0, len2 = card.length; m < len2; m++) {
                    set = card[m];
