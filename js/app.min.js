...
controllers.controller('AdminQuestionCtrl', [
  "$scope", "questionsAPI", "$stateParams", function($scope, questionsAPI, $stateParams) {
    if (!$stateParams.id) {
      $scope.questions = [];
      $scope.languages = questionsAPI.languages();
      return questionsAPI.admin.questions().then(function(response) {
...
services.service('questionsAPI', [
  "$http", "CacheFactory", "$q", "$translate", function($http, CacheFactory, $q, $translate) {
    ...
    return {
      ...
      admin: {
        questions: function() {
          return $http.get(apiURL + "admin-questions");
        },
        question: function(id) {
