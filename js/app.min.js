...
controllers.controller('AdminNewCtrl', [
  "$scope", function($scope) {
    return console.log("AdminNewCtrl");
  }
]);
...
controllers.controller('AdminQuestionCtrl', [
  "$scope", "questionsAPI", "$stateParams", "$window", "$state", function($scope, questionsAPI, $stateParams, $window, $state) {
    ...
    questionsAPI.admin.question($stateParams.id).then(function(response) {
...
controllers.controller('AdminTranslationsCtrl', [
  "$scope", "questionsAPI", function($scope, questionsAPI) {
    $scope.user = questionsAPI.user();
    $scope.selected = {
      language: $scope.user.languages[0] || $scope.languages[1].id,
      search: ""
    };
    if ($scope.user.languages.length) {
      $scope.languages = $scope.user.languages;
    }
...
controllers.controller('AdminTranslationCtrl', [
  "$scope", function($scope) {
    return console.log("AdminTranslationCtrl");
  }
]);

controllers.controller('AdminUserCtrl', [
  "$scope", function($scope) {
    return console.log("AdminUserCtrl");
  }
]);
...
services.service('questionsAPI', [
  "$http", "CacheFactory", "$q", "$translate", function($http, CacheFactory, $q, $translate) {
    ...
    return {
      ...
      logout: function() {
        return caches.session.remove("user");
      },
      auth: function(token) {
        var deferred, url;
        deferred = $q.defer();
      ...
      admin: {
        ...
        "delete": function(id) {
          return $http["delete"](apiURL + "admin-delete&id=" + id);
        },
