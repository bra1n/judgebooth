...
controllers.controller('SideCtrl', [
  "$scope", "questionsAPI", "$ionicScrollDelegate", "$location", "$ionicSideMenuDelegate", function($scope, questionsAPI, $ionicScrollDelegate, $location, $ionicSideMenuDelegate) {
    ...
    $scope.$watch(((function(_this) {
      ...
      return function(isOpen) {
        if (!isOpen && $scope.filteredQuestions.length) {
          return questionsAPI.filter($scope.filter);
        }
      };
    })(this));
    $scope.tab = "filter";
  ...
  "$scope", "questionsAPI", "$stateParams", "$state", "$ionicScrollDelegate", function($scope, questionsAPI, $stateParams, $state, $ionicScrollDelegate) {
    ...
    $scope.$on("$ionicView.enter", function() {
      return questionsAPI.question($stateParams.id).then(function(question) {
        var card, i, len, ref, ref1, ref2, results;
        ...
        for (i = 0, len = ref1.length; i < len; i++) {
          ...
          if ((ref2 = card.layout) === "split" || ref2 === "aftermath") {
          ...
          if (card.layout === 'aftermath' && card.name_en === card.full_name.substr(0, card.name_en.length)) {
            card.layout = 'normal';
          }
