...
/*------- card data --------
// Sets
$sets = json_decode(file_get_contents("http://mtgjson.com/json/SetList.json"));
echo "loaded ".count($sets)." sets\n";
foreach($sets as $set) {
  $query = "name='".$db->real_escape_string($set->name)."',
  code='".$db->real_escape_string($set->code)."',
  releasedate='".$db->real_escape_string($set->releaseDate)."'";
...
foreach($translations as $language=>$translation) {
  ...
  foreach($questionsArray as $row) {
    if($row[1] == "Number") continue;
    if(!$row[2]) continue; # skip questions not marked as "done"
    $id = $row[1];
